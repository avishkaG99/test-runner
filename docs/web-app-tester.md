...
### How to tell whether your plan was used

Phase 1 (`gather-test-context`) stores whatever it decides to run as the variable **`TEST_PLAN`** and records its origin as **`PLAN_SOURCE`**, then hands both to the execution phase. `PLAN_SOURCE` is one of:

| `PLAN_SOURCE` | Meaning |
|---|---|
| `found in PR description` | Your plan was picked up — this is what you want |
| `sourced from acceptance criteria` | Taken from an Acceptance Criteria heading |
| `sourced from bug repro steps` | Azure DevOps bug repro steps (verify mode) |
| `auto-generated by web-app-tester` | **Nothing was found** — the plugin wrote its own |

The report header names the source. If it says *auto-generated*, your plan was not detected: check that the comment was posted before the label, and that it satisfies the detection rules above.

When the plugin auto-generates, it also **posts the generated plan as a comment** before executing — so that comment is itself the evidence that discovery failed.
